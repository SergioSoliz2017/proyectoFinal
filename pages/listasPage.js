...
export class ListasPage {
  constructor(page) {
    ...
    this.filtradorListasButton = this.page.locator(
      "svg.fa-arrow-down-wide-short"
    );
    this.fechaInicioInput = this.page.locator('input[type="date"]').nth(0);
    this.fechaFinInput = this.page.locator('input[type="date"]').nth(1);
    this.cerrarFiltradorButton = this.page.locator(
      'button[class="sc-hFrEEg bRJrsP"]'
    );
    //listas
    this.registrosCantidadSelect = this.page.getByRole("button", {
      name: /Filas por página:/,
    });
    ...
    //editar información
    this.editarInformacionButton = this.page.locator(
      'svg[data-icon="pen-to-square"]'
    );
    this.guardarInformacionButton = this.page.locator(
      'svg[data-icon="floppy-disk"]'
    );

    this.nombreEditarInput = this.page.locator('input[placeholder="Nombre"]');
    this.apellidoEditarInput = this.page.locator(
      'input[placeholder="Apellido"]'
    );
    this.fechaNacimientoEditarInput = this.page.locator(
      'input[placeholder="FechaNacimiento"]'
    );
    this.colegioEditarInput = this.page.locator('input[placeholder="Colegio"]');
    this.direccionEditarInput = this.page.locator(
      'input[placeholder="Dirección"]'
    );
    this.ciudadEditarInput = this.page.locator('input[placeholder="Ciudad"]');
    this.departamentoEditarInput = this.page.locator(
  ...
  async buscarFiltroOptions(optionFiltro) {
    ...
    const filtroOption = this.page.locator(".sc-ddcaxn", {
      hasText: `${optionFiltro}`,
    });
  ...
  async editarInformacionEstudiante(
    codEstudiante,
    nuevoNombre,
    neuvoApellido,
    fechaNacimiento,
    ...
    // boton inf
    const botonInformacion = filaEstudiante.locator(
      'div[title="Información del estudiante"]'
    );
  ...
  async editarInformacionTutor(
    ...
    // boton inf
    const botonInformacion = filaTutor.locator(
      'div[title="Información del tutor"]'
    );
    await expect(botonInformacion.first()).toBeVisible();
    await botonInformacion.click();
  ...
  async asignarCurso() {
    await this.page.locator('div[title="Agregar estudiante"]').first().click();
  }

  async asignarTutor() {
    const selectTutor = this.page.locator("select.sc-BrFsL.bZIwH");
    await expect(selectTutor).toBeVisible();
    await selectTutor.selectOption({ index: 1 });
    const botonSiguiente = this.page.locator("button.sc-hgZZql.iDwqhl", {
      hasText: "Siguiente",
    });
  ...
  async gotoSiguiente() {
    await this.page
      .locator("button.sc-hgZZql.iDwqhl", { hasText: "Siguiente" })
      .click();
  }
}
